...
def compute_portfolio_overview(books_30, books_90, country_30):
    ...
    # Fix: Publisher Champ's country stats use 'converted_spend' for total spend (FB + Amazon)
    # whereas 'spend' is often just Amazon-only spend. We must use the total.
    processed_countries = {}
    for name, c in countries.items():
        processed_countries[name] = c.copy()
        # Use converted_spend (total) if available, otherwise converted
        total_sp = c.get("converted_spend") or c.get("spend_converted") or c.get("spend") or 0
        processed_countries[name]["converted_spend_total"] = total_sp
        ...
        if prev_date:
            ...
            for r in latest_recs:
                ...
                if key and key in prev_map:
                    ...
                    # Fix: Only consider books where BOTH previous and current BSR are in the top 100,000.
                    # This ensures we see momentum among already-selling books, not spikes from obscurity.
                    if prev_bsr and curr_bsr and 0 < prev_bsr <= 100000 and 0 < curr_bsr <= 100000:
                        improvement = prev_bsr - curr_bsr
                        # Also ensure the improvement is significant (at least 10%)
                        if improvement > 0 and (improvement / prev_bsr) >= 0.1:
                            ...
                            movers.append({
                                ...
                                'improvement_pct': (improvement / prev_bsr * 100),
                                'cover_url': book_info.get('image', ''),
                            })
            # Sort by a weighted score: (absolute improvement) * (1 / log10(current_bsr))
            # This prioritizes big moves that end up at a better (lower) rank.
            import math
            movers.sort(key=lambda x: -(x['improvement'] * (1 / math.log10(max(2, x['curr_bsr'])))))
...
def compute_readthrough(series_data, period_label="90-day"):
    ...
    # Sort by total gross revenue to show most commercially important series first
    readthrough_results.sort(key=lambda x: -x["total_gross"])
...
def compute_bsr_revenue_correlation(books_30, bsr_data):
    ...
    for b in books_30:
        ...
        if not asin:
            continue
        series_list = b.get("all_series", []) or []
        title = b["title"]
        
        # Build series label: use first series entry, or detect box set
        is_box = is_boxset_or_omnibus(title)
        
        # Get series name (without position) from first valid series entry
        sname = ""
        valid_series_entry = None
        for s in series_list:
            parts = s.split(" in ", 1)
            if len(parts) == 2:
                candidate = parts[1].strip()
                if candidate:  # must have a non-empty series name
                    sname = candidate
                    valid_series_entry = s
                    break
            elif s.strip() and not s.startswith("#"):
                # No position prefix — use as-is
                sname = s.strip()
                valid_series_entry = s
                break
            
        if is_box:
            series_label = f"Box Set{f' · {sname}' if sname else ''}"
        elif valid_series_entry:
            series_label = valid_series_entry  # e.g. "#1 in Ryan Kaine"
        else:
            series_label = "Standalone"
        
        free_units = b.get("ebooks_free_sold", 0) or 0
        asin_revenue[asin] = {
            "title": title,
    ...
    for asin, readings in asin_bsr_readings.items():
        ...
        free_units = rev.get("free_units", 0) or 0
        gross = rev["gross"]
        
        # Exclude perma-free books: they rank on Amazon's FREE chart, not paid chart
        # Perma-free = has significant free downloads AND near-zero gross revenue
        if free_units > 10 and gross < 10:
            continue
        
        # Flag books with notable free distribution (free + KU)
        is_free_ku = free_units > 0 and rev["kenp"] > 0
...
def generate_html(portfolio, authors, authors_90, series_30, series_90, readthrough_90, readthrough_30,
        ...
        mover_html = f"""
            <div class="mover-kpi-inner">
                ...
                <div class="mover-kpi-text">
                    ...
                    <div style="font-size:0.65rem;color:#888;margin-bottom:0.2rem;font-weight:500;">Among titles ranked in top 100K</div>
                    <div class="kpi-sublabel" style="font-size:0.7rem;white-space:normal;line-height:1.3;margin-top:0.1rem;">{mover_title}</div>
                    {author_line}
    ...
    # Readthrough Funnels (Top 12 series by revenue with RT data)
    rt_charts_html = ""
    # Already sorted by total_gross in compute_readthrough
    ...
    # Ad Efficiency
    # Minimum £1K gross to qualify for Star/Solid tiers
    MIN_GROSS = 1000
    ...
    # Burning Money: ROI < 100% (spending more than revenue). 
    # No minimum gross required — we want to see all loss-makers.
    burning_funnels = [f for f in ad_funnels if f["roi"] < 100]
    ...
    burning_funnels = burning_funnels[:15]
    ...
    for name, cdata in sorted(countries_filtered.items(), key=lambda x: -x[1].get("converted", 0)):
        ...
        spend = cdata.get("converted_spend_total", 0)
        net = gross - spend
    ...
    for c in bsr_correlation[:25]:
        series_lbl = c.get("series_label", "Standalone")
        # Truncate long labels gracefully
        if len(series_lbl) > 45:
            series_lbl = series_lbl[:43] + "…"
        # Badge for free+KU books
        free_badge = ""
        if c.get("is_free_ku"):
            free_badge = ' <span style="font-size:0.6rem;background:#333;color:var(--amber);padding:1px 4px;border-radius:3px;vertical-align:middle;">Free+KU</span>'
        bsr_rev_rows += f"""
        <tr>
            <td class="num mono" style="text-align: right; padding-right: 1.5rem;">{c['bsr_us']:,}</td>
            <td class="book-title">
                <div style="font-weight: 700; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">{c['title']}{free_badge}</div>
                <div style="font-size: 0.7rem; color: var(--text-secondary);">{c['author']}</div>
            </td>
            <td style="font-size: 0.75rem; color: var(--text-secondary); text-align: left; padding-left: 0.5rem;">{series_lbl}</td>
...
<body>
    ...
    <div class="container">
        ...
        <div class="section">
            ...
            <div class="col-header" style="color: var(--red); margin-bottom: 0.5rem;">✗ Burning Money (ROI &lt; 100%)</div>
            <div class="table-wrapper">
                <table class="data-table">
                    <colgroup>
                        <col style="width: 35%;">
